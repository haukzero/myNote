...
- 本质：创建的是一个对象，编译器会将其转换成一个函数对象，并为此生成一个唯一的命名

- 用途：定义较小的回调内联取传递给其他函数
...
- 定义：
  ...
  - `capture`表示外部捕获变量；
    - 捕获后默认类型为`const`
    - 不允许捕获全局变量和静态变量，但可用直接在函数体内直接使用
  - `params`表示形参列表；
  - `specifiers`表示可选限定符，可用`mutable`允许修改`capture`的值；
  - `expction`表示；
  - `ret`表示返回类型；
  - `body`为函数体
...
- <font color=red>ATTENTION</font>:
  ...
  - 调用`mutable`选项后修改的是`capture`的copy版本，原变量的值并未发生改变，但copy版本的值会保留给下次使用
  - 定义lambda表达式后就已经捕获外部变量了，后续外部变量值的改变不会影响lambda表达式内的捕获值(设为引用除外)
...
- 关于修改`capture`，除了调用`mutable`临时修改，也可以将`capture`改为对外部变量的引用，从而直接修改外部变量

### 特殊的捕获方法

- `[this]`：捕获`this`指针，使用`this`类型的成员变量和函数
...
- 简单捕获：即狭义上的lambda表达式中的捕获
- 初始化捕获：捕获表达式结果或自定义捕获变量名

```cpp
//e.g.
int x = 5;
auto fn = [x = x + 1] {return x; };
//第一个x为copy值,第二个为外部的x
auto fnn = [r = x + 2] {return r; };
//定义只存在lambda表达式中的变量r，其值等于外部变量x+2
```
...
C++20提出的新标准，对C++14的泛型lambda表达式做出的改进
...
```cpp
//e.g.
auto fn = []<typename T>(T str) { return str; };
cout << fn("Hello World!") << endl;
```